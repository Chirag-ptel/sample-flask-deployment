...
# Use jq to URL encode
username=$(jq -rn --arg x "jenkinsuser" '$x|@uri')
new_password=$(jq -rn --arg x "jenkinspass" '$x|@uri')
fullname=$(jq -rn --arg x "Jenkins Admin" '$x|@uri')
email=$(jq -rn --arg x "user@jenkins.com" '$x|@uri')
